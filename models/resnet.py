...
class ResNetTypeII(tf.keras.Model):
    def __init__(self, layer_params):
        ...
        self.bn1 = layers.BatchNormalization()
        self.pool1 = layers.MaxPool2D(3, 2, "same")
        self.layer1 = make_bottleneck_layer(64, blocks=layer_params[0])
        self.layer2 = make_bottleneck_layer(128, layer_params[1], 2)
        self.layer3 = make_bottleneck_layer(256, layer_params[2], 2)
        self.layer4 = make_bottleneck_layer(512, layer_params[3], 2)
        self.avgpool = layers.GlobalAveragePooling2D()
        self.fc = layers.Dense(units=NUM_CLASSES, activation=tf.keras.activations.softmax)
...
def resnet_101():
    return ResNetTypeII(layer_params=[2, 4, 23, 3])
